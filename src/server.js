...
import { backupStatus, runGoogleDriveBackup, startBackupScheduler } from './infrastructure/google-drive-backup.js';
...
async function api(req, res, url) {
  if (req.method === 'POST' && url.pathname === '/api/login') {
    ...
    res.writeHead(200,{'content-type':'application/json; charset=utf-8','set-cookie':`session=${encodeURIComponent(token)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800${runtimeConfig.production ? '; Secure' : ''}`}); return res.end(JSON.stringify({user:{id:user.id,display_name:user.display_name,role:user.role}}));
  }
  if (req.method === 'POST' && url.pathname === '/api/logout') { const token=cookies(req).session; if(token) db.prepare('DELETE FROM sessions WHERE token_hash=?').run(tokenHash(token)); res.writeHead(204,{'set-cookie':`session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0${runtimeConfig.production ? '; Secure' : ''}`}); return res.end(); }
  ...
  if(req.method==='GET'&&url.pathname==='/api/calendar'){const start=url.searchParams.get('start');const end=url.searchParams.get('end');const bars=rows(`SELECT pp.id,pp.project_id,pp.planned_start_date start,pp.planned_end_date end,pp.status,p.job_number,p.construction_name,pm.name process_name,0 is_placeholder FROM project_processes pp JOIN projects p ON p.id=pp.project_id JOIN process_masters pm ON pm.id=pp.process_master_id WHERE p.deleted_at IS NULL AND pp.planned_start_date<=? AND pp.planned_end_date>=?
    UNION ALL
    SELECT p.id * -1 id,p.id project_id,COALESCE(NULLIF(p.start_date,''),p.due_date) start,p.due_date end,'planned' status,p.job_number,p.construction_name,'工程未登録' process_name,1 is_placeholder FROM projects p WHERE p.deleted_at IS NULL AND NOT EXISTS (SELECT 1 FROM project_processes pp WHERE pp.project_id=p.id) AND COALESCE(NULLIF(p.start_date,''),p.due_date)<=? AND p.due_date>=?
    ORDER BY start,job_number,is_placeholder`,end,start,end,start);return json(res,200,{bars});}
  ...
  if(req.method==='GET'&&url.pathname==='/api/admin/backup-status'){if(user.role!=='admin')return fail(res,new Error('対象が見つかりません'),404);return json(res,200,{backup:backupStatus()});}
  if(req.method==='POST'&&url.pathname==='/api/admin/backup'){if(user.role!=='admin')return fail(res,new Error('対象が見つかりません'),404);try{const result=await runGoogleDriveBackup(db);audit(user,'BACKUP','database',null,{file_id:result.id});return json(res,200,{ok:true,file_id:result.id});}catch(error){return fail(res,error,500);}}
...
export const server=http.createServer(async(req,res)=>{const url=new URL(req.url,`http://${req.headers.host??'localhost'}`);try{res.setHeader('x-content-type-options','nosniff');res.setHeader('x-frame-options','DENY');res.setHeader('referrer-policy','same-origin');if(url.pathname==='/healthz'){db.prepare('SELECT 1').get();return json(res,200,{ok:true});}if(url.pathname.startsWith('/api/'))await api(req,res,url);else staticFile(req,res,url);}catch(error){console.error(error);if(!res.headersSent)fail(res,error,500);else res.end();}});
if(resolve(process.argv[1]??'')===fileURLToPath(import.meta.url))server.listen(port,'0.0.0.0',()=>{startBackupScheduler(db);console.log(`Wellnot Order Profit Management listening on port ${port}${runtimeConfig.publicBaseUrl ? ` (${runtimeConfig.publicBaseUrl})` : ''}`);});
