...
async function api(req, res, url) {
  ...
  if (req.method === 'PATCH' && projectMatch) {
    ...
    try { assertEditable(project); assertDeadlineChange(project.due_date,input.due_date??project.due_date,input.deadline_change_reason); transaction(()=>{ const due=input.due_date??project.due_date; db.prepare(`UPDATE projects SET construction_name=?,due_date=?,special_notes=?,site_notes=?,version=version+1,updated_by=?,updated_at=? WHERE id=? AND version=?`).run(input.construction_name??project.construction_name,due,input.special_notes??project.special_notes,input.site_notes??project.site_notes,user.id,now(),id,input.version); if(due!==project.due_date) db.prepare('INSERT INTO deadline_changes(project_id,old_due_date,new_due_date,reason,changed_by,changed_at) VALUES(?,?,?,?,?,?)').run(id,project.due_date,due,input.deadline_change_reason.trim(),user.id,now()); audit(user,'UPDATE','project',id,{due_changed:due!==project.due_date}); }); return json(res,200,{project:projectView(one('SELECT p.*,c.name customer_name FROM projects p JOIN customers c ON c.id=p.customer_id WHERE p.id=?',id),user.role)}); } catch(error){ return fail(res,error,409); }
  }
  ...
  if(req.method==='GET'&&url.pathname==='/api/exports/excel'){if(user.role!=='admin')return fail(res,new Error('対象が見つかりません'),404);const start=url.searchParams.get('start'),end=url.searchParams.get('end');try{const grouping=exportGrouping(start,end);const projects=rows(`SELECT p.job_number,c.name customer_name,p.construction_name,p.start_date,p.due_date,p.status FROM projects p JOIN customers c ON c.id=p.customer_id WHERE p.deleted_at IS NULL AND p.start_date BETWEEN ? AND ? ORDER BY p.start_date,p.job_number`,start,end);db.prepare('INSERT INTO export_history(export_type,period_start,period_end,exported_by,exported_at,parameters_json) VALUES(?,?,?,?,?,?)').run('excel',start,end,user.id,now(),JSON.stringify({grouping}));audit(user,'EXPORT','excel',null,{start,end,grouping});const xml=spreadsheetXml(projects,grouping);res.writeHead(200,{'content-type':'application/vnd.ms-excel; charset=utf-8','content-disposition':`attachment; filename="orders-${start}-${end}.xls"`});return res.end(xml);}catch(error){return fail(res,error);}}
  return fail(res,new Error('対象が見つかりません'),404);
}

const esc=(v)=>String(v??'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
function spreadsheetXml(projects,grouping){const headers=['工番','客先','工事名','開始日','納期','状態'];const row=(cells)=>`<Row>${cells.map((c)=>`<Cell><Data ss:Type="String">${esc(c)}</Data></Cell>`).join('')}</Row>`;const sheet=(name,items)=>`<Worksheet ss:Name="${esc(name)}"><Table>${row(headers)}${items.map((p)=>row([p.job_number,p.customer_name,p.construction_name,p.start_date,p.due_date,p.status])).join('')}</Table></Worksheet>`;let sheets;if(grouping==='monthly_sheets'){const grouped=Object.groupBy(projects,(p)=>String(p.start_date).slice(0,7));sheets=Object.entries(grouped).map(([month,items])=>sheet(month,items)).join('');}else{sheets=sheet(grouping==='single_month'?'受注工事明細':'期間合算',projects);}return `<?xml version="1.0"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><DocumentProperties xmlns="urn:schemas-microsoft-com:office:office"><Comments>${grouping}</Comments></DocumentProperties>${sheets}</Workbook>`;}
...
const server=http.createServer(async(req,res)=>{const url=new URL(req.url,`http://${req.headers.host??'localhost'}`);try{if(url.pathname.startsWith('/api/'))await api(req,res,url);else staticFile(req,res,url);}catch(error){console.error(error);if(!res.headersSent)fail(res,error,500);else res.end();}});
server.listen(port,()=>console.log(`Wellnot Order Profit Management: http://localhost:${port}`));
