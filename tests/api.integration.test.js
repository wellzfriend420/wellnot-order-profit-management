...
test('管理者と従業員の機密情報をAPIで分離する',async()=>{
  ...
  response=await request('/api/projects',{cookie:admin,method:'POST',value:{job_number:'TEST-001',customer_id:customer.id,construction_name:'架空工事',short_name:'架空',contract_total_tax_included:600000,start_date:'2026-07-01',material_order_date:'2026-07-02',material_delivery_date:'2026-07-04',due_date:'2026-08-31',drawing_management:true,processes:masters.processes.slice(0,2).map((x,index)=>({process_master_id:x.id,planned_start_date:'2026-07-01',planned_end_date:'2026-07-03',employee_id:index===0?masters.employees[0].id:null}))}});assert.equal(response.status,201);const project=await response.json();
  ...
  assert.equal(adminView.project.contract_total_tax_included,600000);
  assert.equal(adminView.project.billing_schedules[0].amount,600000);
  assert.equal(adminView.project.processes[0].employee_id,masters.employees[0].id);
  ...
  assert.equal((await request(`/api/projects/${project.id}/billing-schedules`,{cookie:admin,method:'POST',value:{label:'出来高1',amount:300000,billing_month:'2026-07',expected_payment_month:'2026-08'}})).status,201);
  const profitManagement=await (await request('/api/profit-management?month=2026-08&period=month',{cookie:admin})).json();
  assert.equal(profitManagement.summary.contract,600000);
  assert.equal(profitManagement.summary.budget,10000);
  assert.equal(profitManagement.summary.actual,18000);
  ...
  assert.equal(exportedWorkbook.getWorksheet('2026-07').getCell('F8').value,600000);
